import {consume, Consumer} from './consumer'

/**
 * Create a proxy handler for the model
 */
const modelHandler = {
...
export default class Model {
   ...
   */
  static find (id) {
    return this.getConsumer().find(id)
  }
   ...
   */
  static create (data) {
    return this.getConsumer().create(data)
  }
  ...
  constructor (data, consumer) {
    this._data = data || {}
    this._consumer = consumer.as(this.constructor) || this.constructor.getConsumer()

    Object.keys(data).forEach((name) => {
      let p = Object.getOwnPropertyDescriptor(this, name) || {}
      let getter = p.get || null
      let setter = p.set || null
